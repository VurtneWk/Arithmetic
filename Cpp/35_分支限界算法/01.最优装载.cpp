...
queue<Node *> que;  // 广度优先遍历子集树的FIFO队列
...
int main() {
    ...
    while (i < n) {          // 当前节点是第i层     开始扩展node节点
        int wt = cw + w[i];  // 处理左孩子，表示选择i节点
        if (wt <= c) {       // 选择物品i以后，其总重量不能超过轮船的容量
            if (wt > bestw) {
                bestw = wt;
            }
            // que.push(Node(cw + w[i], i + 1));  // 活结点孩子入队列
            addLiveNode(cw + w[i], i + 1, node, true);
        }
        // que.push(Node(cw, i + 1));  // 处理右孩子，表示不选择i节点
        ...
        node = que.front();  // 处理完i节点后，它成为死节点，然后出队
        que.pop();
