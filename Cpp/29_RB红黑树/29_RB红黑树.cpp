...
class RBTree {
   public:
    ...
    void insert(const T& val) {
        ...
        // 这时记录了 parent ， cur 是 nullptr 也就是要插入的位置
        // 新插入的叶子结点 颜色都必须是红色
                ...
                if (RED == color(uncle)) {
                    ...
                    */
                    if (right(parent(node)) == node) {
                        // 先指向父节点，旋转后，node就和情况2一样 都指向了叶子结点
                        node = parent(node);
                        leftRotate(node);
                    }
                    // 统一处理情况二
